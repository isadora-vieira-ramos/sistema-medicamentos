...
export default function Home() {
  ...
  const [medicamentosVencidos, setMedicamentosVencidos] = useState<
    string[]
  >([])
  const [medicamentosAVencer, setMedicamentosAVencer] = useState<
    Movimentacao[]
  >([])
      ...
      if (item.tipo == 'entrada' && item.aindaDisponivel > 0) {
        ...
        if (data.getTime() < new Date().getTime()) {
          const vencimento: string = `${
            item.medicamento
          } - Lote com vencimento: ${formatarData(item.validade)}`
          listaMedicamentosVencidos = [
            ...listaMedicamentosVencidos,
            vencimento,
          ]
        } else {
    ...
    <div>
      ...
      <div className='max-w-screen-xl mx-auto p-4'>
        ...
        <div style={{ display: 'flex', flexDirection: 'row' }}>
          ...
          >
            <b>Medicamentos a vencer:</b>
            {medicamentosAVencer.map(item => {
              return (
                <p>{`${item.medicamento} - Lote com vencimento: ${item.validade}`}</p>
              )
            })}
          </div>
          <div
          ...
          >
            <b>Medicamentos com mais estoque:</b>
