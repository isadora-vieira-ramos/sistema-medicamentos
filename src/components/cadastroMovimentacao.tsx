...
export default function CadastroMovimentacao() {
  ...
  useEffect(() => {
    ...
    var profisisonalList =
      obterDoLocalStorage<Profissional>('profissionais')
  ...
  function carregarLotesMedicamento(e: string) {
    setTipo(e)
    const filtrarMovimentos = movimentacoes.filter(
      m =>
      ...
      <div className='max-w-screen-xl mx-auto p-4'>
        ...
        >
          ...
          <select
            ...
            value={tipo}
            onChange={e => carregarLotesMedicamento(e.target.value)}
          >
          ...
          {tipo == 'saida' ? (
            ...
            >
              ...
              {lotesMedicamento.map(m => {
                return (
                  <option value={m.validade}>
                    Lote com validade {formatarData(m.validade)}
                  </option>
                )
              })}
            </select>
          ...
          {tipo == 'entrada' ? (
            <input
              ...
              placeholder='Validade do lote do medicamento'
              required
            />
          ) : (
            <></>
            ...
            <tbody className='w-full bg-white rounded shadow'>
              {movimentacoes.map((m, index) => (
                <tr key={index}>
                  ...
                  <td style={{ textAlign: 'center' }} className='p-2'>
                    {m.tipo == 'entrada' ? m.aindaDisponivel : '-'}
                  </td>
                </tr>
              ))}
            </tbody>
