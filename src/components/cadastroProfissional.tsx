    ...
    <div>
      ...
      <div className='max-w-screen-xl mx-auto p-4'>
        ...
        <form style={{marginBottom: "50px", display:"flex", flexDirection: "column", justifyContent:"left", alignItems: "start"}} onSubmit={handleSubmit}>
          <input style={{borderRadius: "20px", border: "1px solid black", padding: "10px", margin: "10px"}} value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Nome" required />
          <input style={{borderRadius: "20px", border: "1px solid black", padding: "10px", margin: "10px"}} value={especialidade} onChange={(e) => setEspecialidade(e.target.value)} placeholder="Especialidade" required />
        ...
        <div>
          ...
          <table className="w-full bg-white rounded shadow">
            ...
            <tbody className='w-full bg-white rounded shadow'>
              {profissionais.map(p=> (
                <tr key={p.nome}>
                  <td style={{textAlign: 'center'}} className="p-2">{p.nome}</td>
                  <td style={{textAlign: 'center'}} className="p-2">{p.especialidade}</td>
                </tr>
              ))}
            </tbody>
          </table>
